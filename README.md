...
./gradlew jar
```
This will generate the file in `./app/build/libs/app-0.1.2.jar`, which can be
run in the usual way:
```shell
java -jar app/build/libs/app-0.1.2.jar
```
...
./gradlew assemble
```
This will generate archives `app-0.1.2.tar` and `app-0.1.2.zip` of the
