...
./gradlew jar
```
This will generate the file in `./app/build/libs/app-0.0.7.jar`. This can then
be run in the usual way:
```shell
java -jar app/build/libs/app-0.0.7.jar
```
...
./gradlew assemble
```
This will generate archives `app-0.0.7.tar` and `app-0.0.7.zip` of the
